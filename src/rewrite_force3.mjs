...
const startTag = `  const forceCompleteSubDungeon = useCallback((dungeonId: string) => {\n    setDungeons(prevDungeons => {`;
const startIdx = content.indexOf(startTag);
if(startIdx !== -1) {
  const endTag = `    });\n  }, [addXP, addCoins, addRewardToHistory]);`;
  const endIdx = content.indexOf(endTag, startIdx);
  if(endIdx !== -1) {
    const p1 = content.slice(0, startIdx);
    const p2 = content.slice(endIdx + endTag.length);
    ...
    fs.writeFileSync('src/hooks/useGameState.ts', p1 + repStr + p2);
    console.log("Success force slice!");
  } else {
