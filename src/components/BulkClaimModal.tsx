...
import { X, Trophy, Target, Coins, Zap, Star, Gift, CheckCircle2 } from 'lucide-react';
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                ...
                {totals.talentPoints > 0 && (
                  <div className="p-3 bg-purple-500/10 border border-purple-500/20 rounded-2xl flex flex-col items-center justify-center gap-1 text-center">
                    ...
                    <span className="text-[8px] font-bold text-purple-500/60 uppercase tracking-tighter">Talent Points</span>
                  </div>
                )}
                {totals.shards > 0 && (
                      ...
                      <div className="flex items-center gap-1.5">
                        {item.rewards.map((r, ridx) => (
                          <div key={ridx} className="flex items-center gap-1 px-2 py-1 bg-black/40 rounded-full border border-white/5 text-[10px] font-black text-white">
                            {r.type === 'coins' ? <Coins size={12} className="text-amber-400" /> : <Zap size={12} className="text-indigo-400" />}
